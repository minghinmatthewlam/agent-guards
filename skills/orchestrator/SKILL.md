...
Invoke the `new-task` skill. When 95%+ confident, proceed to Phase 2 yourself.

## Phase 2: Plan

Invoke the `plan-loop` skill. When user approves the plan, proceed to Phase 3 yourself.
...
Invoke the `review-loop` skill on the full changeset (including simplify and verify changes). At >=85 confidence: commit, push, and create PR. Include in the PR summary: what was built, key decisions, and residual risks.
