...
Before planning, find what's available to verify on the real surface. Search for test configs, test helpers, build scripts, and existing specs — they show how to build, launch, and interact with the product. If you're missing a tool to reach the product surface, tell the user what you need and why — don't silently downgrade to unit tests.
...
- **Name the exact proof command you'll run.** If an existing test covers the surface, name it. If not, plan to write one — name the test file and the command to run it. If you can't reach the surface at all, surface that as a blocker.
